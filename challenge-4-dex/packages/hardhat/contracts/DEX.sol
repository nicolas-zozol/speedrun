...
contract DEX {
    ...
    uint256 public totalLiquidity; // total amount of liquidity tokens minted
    mapping(address => uint256) public liquidity; // liquidity provided by each LP
    uint256 public ethReserve; // ETH reserve in the DEX
    uint256 public tokenReserve; // Token reserve in the DEX
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        if (msg.value == 0 || tokens == 0) {
            console.log("## Error: Initialization requires both ETH and tokens to be non-zero");
        ...
        if (!success) {
            console.log("## Error: Token transfer failed during initialization");
        ...
        totalLiquidity = msg.value; // Since ratio is 1:1, we can use ETH value
